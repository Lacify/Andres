...
#Ahora utilizamos el while true para hacer un loop infinito solo siendo capaz de ser detenido por el usuario si es deseado, la variable primero es la encargada de que el usuario
#determine lo que quiere hacer a continuacion y de que el usuario pueda salir del loop escribiendo la palabra salir
while(True):
   ...
   if variable_usuario == "salir":
      break
#Aqui usamos int input para que el input pase de ser str a int y se puedan ingresar numeros, luego creamos una lista vacia la cual con el append se ira llenando de la informacion
...
#Esta es la unica distinta, aqui solo utilizamos un valor y ese valor una vez brindado por el usuario la funcion factorial le sacara el mismo factorial al numero.
      ...
      a = int(input("ingrese el primer numero del factorial: "))
      if a < 0:
